...
def takeInput():
    ...
    inputValues.append(rbp.get())
    inputValues.append(chol.get())
    inputValues.append(fbs.get())
    inputValues.append(recg.get())
    inputValues.append(thalach.get())
    inputValues.append(exang.get())
    inputValues.append(oldpeak.get())
    ...
    print("\n") 
    final_Result = scv.predict([inputValues])
    print(final_Result)
...
heart = pd.read_csv("cleaned_heart_dataset.csv")
y = heart['target']
...
chol = tkinter.Entry(serumCholFrame)
chol.grid(row=2, column=2, sticky='n')

FBSFrame = tkinter.LabelFrame(mainWindow, text="Fasting BS(0-4)")
...
recg = tkinter.Entry(ECGFrame)
recg.grid(row=2, column=2, sticky='nw')
